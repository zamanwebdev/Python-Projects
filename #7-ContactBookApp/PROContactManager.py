...
def delete_contact():
    ...
    if name in contacts:
        confirm = messagebox.askyesno("Confirm", f"Delete {name.title()}?")
        if confirm:
            del contacts[name]
            save_contacts()
            update_list()
...
def edit_contact():
    ...
    name = selected.split(" : ")[0].lower()
    if name in contacts:
        old_numbers = contacts[name]
        new_numbers = simpledialog.askstring("Edit Numbers",
                                             f"Current numbers: {', '.join(old_numbers)}\nEnter new numbers (comma separated):")
        if new_numbers:
            new_list = [num.strip() for num in new_numbers.split(",") if num.strip().isdigit()]
            if new_list:
                contacts[name] = new_list
                save_contacts()
                update_list()
                messagebox.showinfo("Success", "Numbers updated")
            else:
...
def main_window():
    ...
    tk.Label(root, text="📒 PRO Contact Manager", font=("Arial", 18, "bold"), bg="#1e1e2f", fg="white").pack(pady=10)
    ...
    tk.Entry(input_frame, textvariable=name_var, width=30).grid(row=0, column=1, padx=5)

    tk.Label(input_frame, text="Phone", bg="#2c2c3e", fg="white").grid(row=1, column=0, padx=5, pady=5)
    tk.Entry(input_frame, textvariable=phone_var, width=30).grid(row=1, column=1, padx=5)
    ...
    tk.Button(btn_frame, text="Edit Numbers", width=12, bg="#9C27B0", fg="white", command=edit_contact).grid(row=0, column=3, padx=5)
    tk.Button(btn_frame, text="Export Excel", width=12, bg="#FFC107", fg="white", command=export_excel).grid(row=0, column=4, padx=5)
    tk.Button(btn_frame, text="Exit", width=12, bg="#607D8B", fg="white", command=root.destroy).grid(row=0, column=5, padx=5)

    # Listbox Frame
    list_frame = tk.Frame(root)
    list_frame.pack(pady=10)

    contact_list = tk.Listbox(list_frame, width=70, height=15, font=("Arial", 10))
